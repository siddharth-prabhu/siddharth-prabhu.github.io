...
where $x \in \mathbb{R}^n$ is the decision variable, $p \in \mathbb{R}^p$ are some learnable parameters, $f : \mathbb{R}^n \times \mathbb{R}^p \to \mathbb{R}$ is the objective function, $ g : \mathbb{R}^n \times \mathbb{R}^n \to \mathbb{R}^g $ are the equality constraints and $h : \mathbb{R}^n \times \mathbb{R}^n \to \mathbb{R}^h $ are the inequality constraints. For the optimization problem to be convex in $x$, the objective function $f$ and the inequality constraints $h$ have to be convex in $x$, while the equality constraints $g$ must be affine in $x$. Let $x^{*} $ denote the primal optimal solution, and let $[\lambda ^{*} \in \mathbb{R}^g, \mu ^* \in \mathbb{R}^h]$ denote the dual optimal solution, corresponding to the equality and inequality constraint respectively. We define the Lagrangian of this optimization problem as 
...
where $ L_x$ is the gradient of the objective function with respect to $x$. Then the optimal solution $z^{*} = [x^{*}, \lambda ^{*}, \mu ^{*}]$ is such that $R(z^{*}, p) = 0$, $\ h(x^{*}, p) \leq 0 $, and $\mu ^* \geq 0 $. We will also assume that the optimal solution is regular, i.e. a small perturbation is the optimal solution, does not change the active set ($ \left\{ i \mid h_i(x^*, p) = 0 \right\} $). Consequently, using the implicit function theorem gives
...
where we need to find $ w_1 = \frac{dx}{dp}v, \ w_2 = \frac{d\lambda}{dp}v, \ w_3 = \frac{d\mu}{dp}v $. On the other hand, $v_1 = L_{xp}v, \ v_2 = g_{p}v, \ v_3 = \text{diag}(\mu ^*)h_{p}v$ are already expressed in a form that can be efficiently computed using jvp in JAX. The corresponding system of equations can be explicitly written as 
